...
from models import convolutional_model, convolutional_model_simple, recurrent_model,VGG16_convolutional_model
...
from pre_process import find_files
...
def main(libri_dir=c.DATASET_DIR):
    ...
    # Looking for .npy files in DATASET_DIR.
    logging.info('Looking for fbank features [.npy] files in {}.'.format(libri_dir))
    # from libri_dir you can find some speakers and the total files with .npy
    ...
    if len(libri) == 0:
        ...
        #process turning the wav audio from c.WAV_DIR to npy file in DATASET_DIR
        ...
        if len(libri) == 0:
            ...
            exit(1)
    #return the speakers id in a list.
    unique_speakers = libri['speaker_id'].unique()
    #creating a dict like {19:[xxxx.npy,xxxx.npy]},and unique_speakers is still a list.
    spk_utt_dict, unique_speakers = create_dict(libri['filename'].values,libri['speaker_id'].values,unique_speakers)
    #using multi-thread to create stack
    select_batch.create_data_producer(unique_speakers, spk_utt_dict)

    #select c.BATCH_SIZE data from libri  positive batch, nagetive batch , anchor batch
    ...
    #convert the data to (value of audio) and label.x -> value of audio y->label
    x, y = batch.to_inputs()
    b = x[0]#160*64*1
    #defination of number of frame
    ...
    input_shape = (num_frames, b.shape[1], b.shape[2])   #160*64*1
    ...
    orig_time = time()
    #construct the RES-CNN model using the input_shape and batchsize and num_Frams
    model = convolutional_model(input_shape=input_shape, batch_size=batch_size, num_frames=num_frames)
    # model = VGG16_convolutional_model(input_shape=input_shape, batch_size=batch_size, num_frames=num_frames)
    logging.info(model.summary())
    gru_model = None
    if c.COMBINE_MODEL:#whether it is a combine model
    ...
    if PRE_TRAIN:
        last_checkpoint = get_last_checkpoint_if_any(c.PRE_CHECKPOINT_FOLDER)#getting .h5 file from PRE_CHECKPOINT_FOLDER
    ...
    while grad_steps <= epoch:
        ...
        if (grad_steps) % 10 == 0:
            fm1, tpr1, acc1, eer1 = eval_model(model, train_batch_size, test_dir=c.VCTK_TEST_DIR, check_partial=True, gru_model=gru_model)
        ...
        if (grad_steps ) % c.TEST_PER_EPOCHS == 0 :
            fm, tpr, acc, eer = eval_model(model,train_batch_size, test_dir=c.VCTK_TEST_DIR,gru_model=gru_model)
